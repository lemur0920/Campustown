 ...
 */
@Component
public class VolunteerReservationValidator implements AsaproValidator{
	...
	public boolean supports(Class<?> clazz) {
		return clazz.isAssignableFrom(VolunteerReservation.class);
	}
	...
	public void validate(Object target, Errors errors, String formMode) {
		VolunteerReservation volunteerReservationForm = (VolunteerReservation)target;
		...
		if("INSERT".equalsIgnoreCase(formMode) || "UPDATE".equalsIgnoreCase(formMode) ){
			...
			ValidationUtils.rejectIfEmptyOrWhitespace(errors, "reservGroup", null, "신청자 그룹은 필수입력입니다.");
			...
			if( StringUtils.isNotBlank(volunteerReservationForm.getReservHp2()) ){
				if(!AsaproUtils.isMobile(volunteerReservationForm.getReservHp2(),"MOBILE2") ){
					errors.rejectValue("reservHp2", null, "휴대폰번호 중간자리는 숫자 3자리 또는 숫자 4자리 형태로 입력해야 합니다.");
				}
			}
			if( StringUtils.isNotBlank(volunteerReservationForm.getReservHp3()) ){
				if(!AsaproUtils.isMobile(volunteerReservationForm.getReservHp3(),"MOBILE3") ){
			...
			if( StringUtils.isNotBlank(volunteerReservationForm.getReservEmail()) ){
				if( !AsaproUtils.isEmail(volunteerReservationForm.getReservEmail()) ){
			...
			ValidationUtils.rejectIfEmptyOrWhitespace(errors, "reservDate", null, "봉사일은 필수입력입니다.");
			ValidationUtils.rejectIfEmptyOrWhitespace(errors, "reservStime", null, "봉사시작시간은 필수입력입니다.");
			ValidationUtils.rejectIfEmptyOrWhitespace(errors, "reservEtime", null, "봉사종료시간은 필수입력입니다.");
			ValidationUtils.rejectIfEmptyOrWhitespace(errors, "reservPurpose", null, "목적은 필수입력입니다.");
			//ValidationUtils.rejectIfEmptyOrWhitespace(errors, "reservSuggest", null, "건의사항은 필수입력입니다.");
			...
			if( volunteerReservationForm.getReservAgree() == null || !volunteerReservationForm.getReservAgree() ){
