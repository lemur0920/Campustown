...
public static StringBuilder buildMenuMarkup(List<Menu> menuList, Menu menu, List<Menu> currentMenuList, Menu currentMenu, int gnbDisplayDepth, StringBuilder sb){
	...
	if( menu.getMenuPosition(menuList) == Menu.FIRST_CHILD || menu.getMenuPosition(menuList) == Menu.ONLY_CHILD){
		if(menu.getMenuDepth() == 1){
			sb.append("<ul class=\"deapth1 clearfix\">");
			//sb.append("<li><a href='#n'><span>캠퍼스타운 소개</span></a>");
		}else if(menu.getMenuDepth() == 2){
			sb.append("<div class=\"deapth2\"><ul>");
		}else if(menu.getMenuDepth() == 3){
			sb.append("<ul class=\"deapth3\">");
		}
	}
	...
	if( "text".equals(menu.getMenuGnbType()) ){
			sb.append(menu.getMenuName());
	} else if( "image".equals(menu.getMenuGnbType()) ) {
		sb.append("<img src=\"");
		if( currentMenuList != null && currentMenuList.contains(menu) ){
			sb.append(menu.getMenuGnbImageOnUrl());
		} else {
			sb.append(menu.getMenuGnbImageOnUrl());
		}
		sb.append("\" alt=\"");
		...
		if( currentMenuList != null && !currentMenuList.contains(menu) ){
			...
			sb.append(menu.getMenuGnbImageOffUrl());
			sb.append("'\"");
		}
		sb.append("/>");
	}
...
int gnbDisplayDepth = Integer.parseInt( StringUtils.defaultString(designConfig.getOption("nav_gnb_display_depth"), "2") );
