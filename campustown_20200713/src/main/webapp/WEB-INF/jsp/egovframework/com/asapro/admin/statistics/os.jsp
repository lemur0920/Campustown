...
jQuery(function($){
	...
	$('.datepicker-ui').datepicker();	
	 ...
	 var $data  = [
		<c:if test="${not empty statisticsOsMapList }">
			<c:forEach items="${statisticsOsMapList }" var="item" varStatus="vs">
				[ '${item.OS_KEY}', ${item.CNT_BY_OS } ]<c:if test="${!vs.last }">,</c:if>
  ...
  //목록에 차트의 색과 동일하게 색처리
    ...
    <c:if test="${not empty statisticsOsMapList }">
 		<c:forEach items="${statisticsOsMapList }" var="item" varStatus="vs">
 		    $('.item${vs.count}').css("color", chartPieColors['${item.OS_KEY}']);
 		</c:forEach>
 	</c:if>
});
</script>
					    ...
					    <div class="col-sm-12">
					        <div class="card m-b-15">
					            <div class="card-body">
						            ...
						            <form:form modelAttribute="statisticsSearch" cssClass="form-inline" action="${ADMIN_PATH}/statistics/os.do?${statisticsSearch.queryString }" method="get">
					...
					<div class="row">
								...
								<div class="card-body">
						            ...
						            <div class="table-responsive">
										<table class="table table-striped table-hover">
											...
											<tbody>
												<c:if test="${empty statisticsOsMapList}">
													<tr>
												...
												<c:forEach items="${statisticsOsMapList}" var="item" varStatus="vs">
													<tr>
														<td>${vs.count}</td>
														<td class="item${vs.count }">${item.OS_KEY}</td>
														<td>${item.CNT_BY_OS}</td>
						...
						<div class="col-lg-6">
							<div class="card m-b-30">
								...
								<div class="card-body">
                                     <p class="text-muted m-b-30 font-14">OS별 세션통계를 보여줍니다.</p>
