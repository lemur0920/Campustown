...
public static StringBuilder buildMenuMarkup(List<Menu> topMenuList, Menu menu, List<Menu> currentMenuList, Menu currentMenu, int snbDisplayDepth, StringBuilder sb){
	//메뉴 출력 깊이 설정값 제한 체크
	if( menu.getMenuDepth() > snbDisplayDepth ){
		return sb;
	}
	//메뉴 상태 체크(참고로 메뉴상태에는 public, hidden, locked 가 있음. 표시는 public 만 함)
	if( "locked".equals(menu.getMenuStatus()) ){
		return sb;
	}
	
	Collections.reverse(currentMenuList);
	
	if( menu.getMenuDepth() == 1 ){
		sb.append("<ul class=\"nav-ul clearfix\">");
		
		sb.append("<li class=\"home\">");
		sb.append("<a href=\"/\">");
		sb.append("<img src=\"" + themePath + "/images/sub/home_ico.gif\" alt=\"홈\">");
		sb.append("</a></li>");
		...
		sb.append("<ul>");
		//모든 메뉴중 1데스 메뉴만 마크업 생성
		for(Menu topMenu : topMenuList){
			if( "public".equals(topMenu.getMenuStatus()) ){
				if( topMenu.getMenuDepth() == 1 ){
					sb.append("<li>");
					sb.append("<a href=\"");
					sb.append(topMenu.getMenuLink());
					sb.append("\"");
					if( topMenu.isMenuNewWin() ){
						sb.append(" title=\"새창\"");
						sb.append(" target=\"_blank\"");
					}
					sb.append(">");
					sb.append(topMenu.getMenuName());
					sb.append("</a>");
					sb.append("</li>");
				}
			}
	...
	if(currentMenu.getMenuDepth() >= 2){
		...
		for(Menu locationMenu : currentMenuList){
			...
			if(locationMenu.getMenuChildren() != null && !locationMenu.getMenuChildren().isEmpty() ){
				...
				sb.append("<a href=\"#none\">");
				sb.append("<span>");
				for(Menu locationMenuN : currentMenuList){
					if(locationMenuN.getMenuDepth() == currentDepth){
						sb.append(locationMenuN.getMenuName());
					}
				}
				sb.append("</span>");
				sb.append("</a>");
				...
				sb.append("<ul>");
				//뎁스별 메뉴만 마크업 생성
				for(Menu depsNMenu : locationMenu.getMenuChildren() ){
					if( "public".equals(depsNMenu.getMenuStatus()) ){
						sb.append("<li>");
						sb.append("<a href=\"");
						sb.append(depsNMenu.getMenuLink());
						sb.append("\"");
						sb.append(" class=\"");
						if( currentMenuList != null && currentMenuList.contains(depsNMenu) ){
							sb.append("on");
						}
						sb.append("\"");
						if( depsNMenu.isMenuNewWin() ){
							sb.append(" title=\"새창\"");
							sb.append(" target=\"_blank\"");
						}
						sb.append(">");
						sb.append(depsNMenu.getMenuName());
						sb.append("</a>");
						sb.append("</li>");
					}
				}
...
themePath = (String)((Map<String, Object>)request.getAttribute("design")).get("resource");
...
@SuppressWarnings("unchecked")
List<Menu> currentMenuList = (List<Menu>)request.getAttribute("currentMenuList");
...
for( Menu m1 : topMenuList ){
	if( topAncestorMenu != null ){
		if( m1.getMenuId().equals(topAncestorMenu.getMenuId()) ){
			buildMenuMarkup(topMenuList, m1, currentMenuList, currentMenu, snbDisplayDepth, sb);
		}
	}
}
