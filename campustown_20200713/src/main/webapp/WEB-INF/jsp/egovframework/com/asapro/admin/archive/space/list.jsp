					...
					<div class="row">
					    <div class="col-sm-12">
					        <div class="card m-b-15">
					            <div class="card-body">
						            <form:form modelAttribute="spaceSearch" cssClass="" action="${ADMIN_PATH}/archive/space/list.do" method="get">
										...
										<div class="form-row mr-auto mt-2 form-inline">
											...
											<form:select path="sortOrder" cssClass="form-control mr-sm-2 mb-2" title="정렬기준">
												<form:option value="ARC_LAST_UPDATE" label="최근 수정일" />
												<form:option value="ARC_TITLE" label="시설공간 명" />
										...
										<div class="form-row mt-2 form-inline" >
											...
											<c:if test="${not empty arcMetaCode1List || not empty arcMetaCode2List || not empty arcMetaCode3List}">
												...
												<form:select path="metaCode2" cssClass="form-control mr-sm-2 mb-2" title="지역(시군구)">
												...
												<form:select path="univId" cssClass="form-control mr-sm-2 mb-2 scUniv" title="캠퍼스타운 ID">
											...
											<%-- 
											<form:select path="arcSelected1" cssClass="form-control mr-sm-2 mb-2" title="일자리카페추출 여부">
												<option value="">일자리카페추출 여부</option>
												<form:option value="true" label="추출" />
												<form:option value="false" label="미추출" />
											</form:select>
											 --%>
											<form:select path="arcUse" cssClass="form-control mr-sm-2 mb-2" title="노출여부">
											...
											<form:select path="sc" cssClass="form-control mr-sm-2 mb-2" title="검색조건">
												...
												<form:option value="arcTitle" label="시설공간 명" />
							...
							<div class="card m-b-30">
								<div class="card-body">
											...
											<tbody>
												...
												<c:forEach items="${paging.result}" var="item" varStatus="vs">
													<tr>
														...
														<%-- 
														<c:if test="${not empty arcMetaCode2ListAll }">
															<td>${asapro:codeName(item.arcMetaCode2, arcMetaCode2ListAll)}</td>
														</c:if>
														 --%>
														<c:if test="${not empty archiveCategory.catMetaCode3 }">
														...
														<td>
														 ...
														 --%>
														<td><a href="${ADMIN_PATH}/archive/space/update.do?arcId=${item.arcId}" class=""><strong>${item.arcTitle}</strong></a></td>
														<td>
														...
														<td>
														 ...
														 --%>
														<td>
															<fmt:formatDate value="${item.arcLastUpdate}" pattern="yyyy-MM-dd" />
														</td>
														<c:if test="${archiveCategory.catSupportSelect1}">
															<td>
									...
									<div class="form-inline">
										<div class="mr-auto">
											<a href="${ADMIN_PATH}/archive/space/insert.do?arcCategory=${archiveCategory.catId}" class="btn btn-primary waves-effect waves-light">추가</a>
