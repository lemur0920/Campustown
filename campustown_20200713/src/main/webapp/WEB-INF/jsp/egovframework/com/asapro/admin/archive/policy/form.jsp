					    ...
					    <div class="col-sm-12">
					        <div class="card m-b-15">
					            <div class="card-body">
					            	...
					            	<c:if test="${formMode eq 'INSERT'}"><c:set var="actionUrl" value="${ADMIN_PATH}/archive/policy/insert.do" /></c:if>
									<c:if test="${formMode eq 'UPDATE'}"><c:set var="actionUrl" value="${ADMIN_PATH}/archive/policy/update.do" /></c:if>
						            <form:form modelAttribute="policyForm" cssClass="" action="${actionUrl}" method="post" enctype="multipart/form-data">
										...
										<div class="form-group row">
											<form:label path="poliShortDescription" cssClass="col-sm-2 col-form-label">간단설명 </form:label>
											<div class="col-sm">
												<form:textarea path="poliShortDescription" cssClass="form-control col-sm-8" rows="3" placeholder="" />
												<form:errors path="poliShortDescription" element="div" cssClass="text-danger col-form-label ml-2" />
											</div>
										</div>
										...
										<c:if test="${not empty archiveCategory.catMetaCode1 }">
											...
											<div class="form-group row">
												...
												<div class="col-sm col-form-label">
													<c:forEach items="${arcMetaCode1List }" var="metaCode1" varStatus="vs"> 
														<c:set var="chksel" value="" />
														<c:forTokens items="${policyForm.arcMetaCode1 }" delims="," var="selectCode">
															<c:if test="${selectCode eq metaCode1.codeId }">
																<c:set var="chksel" value="checked" />
															</c:if>
														</c:forTokens>
														
														<div class="custom-control custom-checkbox custom-control-inline">
															<input type="checkbox" name="arcMetaCode1" data-dayidx="${vs.index }" id="arcMetaCode1${vs.count }" class="custom-control-input" value="${metaCode1.codeId }" ${chksel } />
															<label for="arcMetaCode1${vs.count }" class="custom-control-label"> ${metaCode1.codeName }</label>
														</div>
													</c:forEach>
										...
										<c:if test="${not empty archiveCategory.catMetaCode2 }">
											...
											<div class="form-group row arcMetaCode2">
												<form:label path="arcMetaCode2" cssClass="col-sm-2 col-form-label">${arcMetaCode2Info.catName } <span class="text-danger">*</span></form:label>
												<div class="col-sm form-inline">
													<c:forEach items="${arcMetaCode2List }" var="metaCode2" varStatus="vs"> 
														<c:set var="chksel" value="" />
														<c:forTokens items="${policyForm.arcMetaCode2 }" delims="," var="selectCode">
															<c:if test="${selectCode eq metaCode2.codeId }">
																<c:set var="chksel" value="checked" />
															</c:if>
														</c:forTokens>
														
														<div class="custom-control custom-checkbox custom-control-inline">
															<input type="checkbox" name="arcMetaCode2" data-dayidx="${vs.index }" id="arcMetaCode2${vs.count }" class="custom-control-input" value="${metaCode2.codeId }" ${chksel } />
															<label for="arcMetaCode2${vs.count }" class="custom-control-label"> ${metaCode2.codeName }</label>
														</div>
													</c:forEach>
													<form:errors path="arcMetaCode2" element="div" cssClass="text-danger col-form-label ml-2" />
												</div>	
										...
										<c:if test="${not empty archiveCategory.catMetaCode3 }">
											...
											<div class="form-group row">
												<form:label path="arcMetaCode3" cssClass="col-sm-2 col-form-label">${arcMetaCode3Info.catName } <span class="text-danger">*</span></form:label>
												<div class="col-sm form-inline">
													<c:forEach items="${arcMetaCode3List }" var="metaCode3" varStatus="vs"> 
														<c:set var="chksel" value="" />
														<c:forTokens items="${policyForm.arcMetaCode3 }" delims="," var="selectCode">
															<c:if test="${selectCode eq metaCode3.codeId }">
																<c:set var="chksel" value="checked" />
															</c:if>
														</c:forTokens>
														
														<div class="custom-control custom-checkbox custom-control-inline">
															<input type="checkbox" name="arcMetaCode3" data-dayidx="${vs.index }" id="arcMetaCode3${vs.count }" class="custom-control-input" value="${metaCode3.codeId }" ${chksel } />
															<label for="arcMetaCode3${vs.count }" class="custom-control-label"> ${metaCode3.codeName }</label>
														</div>
													</c:forEach>
									 ...
									 --%>	
										...
										<c:if test="${archiveCategory.catSupportImage }">
											<div class="form-group row">
												...
												<div class="col-sm">
													<div>
														<c:if test="${not empty policyForm.thumb && policyForm.thumb.fileId > 0}">
															<img class="img-thumbnail" style="max-width: 200px; max-height: 200px;" src="${policyForm.thumb.fileServletThumbnailUrl}" onerror="this.src='${CONTEXT_PATH }/assets/images/noImage300x300.jpg'" />
														</c:if>
													</div>
													<div>
											...
											<div class="form-group row">
												...
												<div class="col-sm">
													...
													<div class="mt-2" id="fileList">
														<c:forEach items="${policyForm.arcFileInfos}" var="arcFileInfo" varStatus="vs">
															<div class="file_add">
										...
										<div class="form-group">
											...
											<a href="${ADMIN_PATH}/archive/policy/list.do?arcCategory=${policyForm.arcCategory}" class="btn btn-secondary waves-effect m-l-5">목록</a>
										</div>
									</form:form>
					            </div>
					        </div>
...
CKEDITOR.replace( 'arcContent',{
	filebrowserImageUploadUrl : GLOBAL.ADMIN_PATH + '/file/editor/upload.do?module=archive&moduleId=${archiveCategory.catCustomType}&moduleSub=${archiveCategory.catId}&moduleSubId=${policyForm.arcId}'
} );
</script>	
