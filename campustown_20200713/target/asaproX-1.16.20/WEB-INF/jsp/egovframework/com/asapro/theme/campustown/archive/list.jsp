	...
	<div class="s43buttn wdes33">
		<ul>
			<c:forEach items="${arcMetaCode1List }" var="item">
				<li class="<c:if test="${item.codeId eq archiveSearch.metaCode1 }">on</c:if>"><a href="?metaCode1=${item.codeId }">${item.codeName }</a></li>
			</c:forEach>
		</ul>
		<div class="clear"></div>
	</div>
	<div class="s1marg30">
		<form:form modelAttribute="archiveSearch" action="" method="get">
			...
			<table class="tablest1 btmso">
				...
				<tbody>
					<c:if test="${archiveSearch.metaCode1 ne 'outdoor' }">
					<tr>
						<th>제작년도</th>
						<td class="lplstud"><form:input path="arcYear" /></td>
						<c:choose>
							<c:when test="${archiveSearch.metaCode1 ne 'radio' }">
								<td></td>
								<td></td>
							</c:when>
							<c:when test="${archiveSearch.metaCode1 eq 'radio' }">
								<th>성우</th>
					...
					<c:if test="${archiveSearch.metaCode1 eq 'print' or archiveSearch.metaCode1 eq 'other' }">
					<tr>
						<c:choose>
							<c:when test="${archiveSearch.metaCode1 eq 'print' }">
								<th>구분</th>
							...
							<c:when test="${archiveSearch.metaCode1 eq 'other' }">
								<th>매체</th>
								<td class="lplstud"><form:input path="arcMedia" /></td>
								<td></td>
								<td></td>
					...
					<tr>
						...
						<c:choose>
							<c:when test="${archiveSearch.metaCode1 ne 'outdoor' }">
								<th>제품명</th>
								<td class="lplstud"><form:input path="arcProduct" /></td>
							</c:when>
							<c:when test="${archiveSearch.metaCode1 eq 'outdoor' }">
								<td></td>
								<td></td>
							</c:when>
						</c:choose>
	...
	<div class="s1marg30">
		<table class="tablest5">
			...
			<colgroup>
				...
				<c:if test="${archiveSearch.metaCode1 eq 'print' }">
					<col /><!-- 구분 -->
				</c:if>
				<c:if test="${archiveSearch.metaCode1 eq 'print' or archiveSearch.metaCode1 eq 'other' }">
					<col /><!-- 매체 -->
				</c:if>
				<c:if test="${archiveSearch.metaCode1 eq 'video' or archiveSearch.metaCode1 eq 'radio' or archiveSearch.metaCode1 eq 'outdoor' }">
					<col /><!-- 제목 -->
				</c:if>
				<col /><!-- 광고주 -->
				<c:if test="${archiveSearch.metaCode1 ne 'outdoor' }">
				...
				<c:if test="${archiveSearch.metaCode1 eq 'print' or archiveSearch.metaCode1 eq 'outdoor' or archiveSearch.metaCode1 eq 'media' or archiveSearch.metaCode1 eq 'other' }">
					<col /><!-- 내용 -->
				</c:if>
				<c:if test="${archiveSearch.metaCode1 eq 'media' }">
					<col /><!-- 기간 -->
				</c:if>
			</colgroup>
			<thead>
