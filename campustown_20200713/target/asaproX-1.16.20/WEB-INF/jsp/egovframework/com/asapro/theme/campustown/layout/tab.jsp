...
public static StringBuilder buildMenuMarkup(List<Menu> menuList, Menu menu, List<Menu> currentMenuList, Menu currentMenu, int snbDisplayDepth, StringBuilder sb){
	...
	for(Menu locationMenu : currentMenuList){
		if(locationMenu.getMenuDepth() == 3){
			if(locationMenu.getMenuChildren() != null && !locationMenu.getMenuChildren().isEmpty() ){
				int publicChk = 0;
				sb.append("<div class=\"subl-tab\">");
				sb.append("<ul class=\"clearfix\">");
				for(Menu deps4Menu : locationMenu.getMenuChildren() ){
					if( "public".equals(deps4Menu.getMenuStatus()) ){
						sb.append("<li class=\"");
						if( currentMenuList != null && currentMenuList.contains(deps4Menu) ){
							sb.append("on");
						}
						sb.append("\">");
						sb.append("<a");
						sb.append(" href=\"");
						sb.append(deps4Menu.getMenuLink());
						sb.append("\"");
						if( deps4Menu.isMenuNewWin() ){
							sb.append(" title=\"새창\"");
							sb.append(" target=\"_blank\"");
						}
						sb.append(">");
						sb.append(deps4Menu.getMenuName());
						sb.append("</a>");
						sb.append("</li>");
...
int snbDisplayDepth = Integer.parseInt( StringUtils.defaultString("4", "4") );	//탭메뉴 4depth 임의 처리
...
for( Menu m1 : topMenuList ){
	if( topAncestorMenu != null ){
		if( currentMenu.getMenuDepth() > 2 && m1.getMenuId().equals(topAncestorMenu.getMenuId()) ){//해당 4뎁스메뉴의 부모 3뎁스 메뉴를 전달하여 자식메뉴 4뎁스만 출력
