...
jQuery(function($){
	...
	// 대학목록검색 - 검색버튼
	$('.startSearchBtnLayer').on('click', function(){
		var params = $('#departmentSearch').serialize();
		$('#startList').load('${ADMIN_PATH}/startHpMngr/searchStartLayer.do', params);	
	});
	...
	//대학목록검색 - 페이징 버튼
	$('.contentPasing2 .page-link').on('click', function(e){
		e.preventDefault();
		$('#startList').load($(this).attr('href'));	
	});
	
	
	//콘텐츠 선택시 input 셋팅
	$('.depSelect2').on('click', function(e){
		e.preventDefault();
		
		$('#compId').val($(this).data('depid2'));
		$('#compNm').val($(this).data('depname2'));
		...
		$('#ment2').val($(this).data('depdescription2'));
		$('#depUse2').val($(this).data('depuse2'));	;
            ...
            <div class="card-body">
	            <form:form modelAttribute="departmentSearch" cssClass="form-inline" action="${ADMIN_PATH}/univHpMngr/searchUnivLayer.do" method="get">
					...
					<div class="form-row mt-2" >
						...
						<button class="btn btn-outline-success waves-effect mb-2 startSearchBtnLayer" type="button">검색</button>
					</div>
				</form:form>
            </div>
        </div>
						...
						<tbody>
							...
							<c:forEach items="${paging.result}" var="item" varStatus="vs">
								<tr>
									...
									<td>${item.depId}</td>
									<td><a href="#non" class="depSelect2" data-depid2="${item.depId}" data-depname2="${item.depName}" data-deptel="${item.depTel}" data-depdescription2="${item.depDescription}" data-depuse2="${item.depUse}" ><strong>${item.depName}</strong></a></td>
