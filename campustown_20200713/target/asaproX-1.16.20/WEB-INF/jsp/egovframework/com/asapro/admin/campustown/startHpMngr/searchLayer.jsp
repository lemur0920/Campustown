...
*/
	...
	$('.memberSelect').on('click', function(e){
		...
		var attr = $("#attr").val(); 
		$("input:text[id='" + attr + "']").val($(this).data('adminid'));
		$.magnificPopup.close();
						...
						<tbody>
							...
							<c:forEach items="${paging.result}" var="item" varStatus="vs">
								<tr>
									...
									<td><a href="#non" class="memberSelect" data-name="${item.adminName}" data-tel="${item.adminTelJoin}" data-department="${item.department.depName}" data-email="${item.adminEmail}" data-adminid="${item.adminId}"><strong>${item.adminName}</strong></a></td>
