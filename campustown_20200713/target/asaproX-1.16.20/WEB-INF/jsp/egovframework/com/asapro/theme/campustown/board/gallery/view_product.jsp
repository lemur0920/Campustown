...
<c:import url="${design.themeDir}/layout/sidebar.jsp" charEncoding="UTF-8" />
	...
	<div class="table-con">
		<table>
			...
			<tbody>
				<tr>
					<th scope="row">캠퍼스타운명</th>
					<td>
						<c:if test="${not empty boardArticle.baCategory2}">
							${asapro:depName(boardArticle.baCategory2, departmentList) }
						</c:if>
					</td>
					<th scope="row">창업팀명</th>
					<td>
						<c:if test="${not empty boardArticle.baCategory3}">
							<c:forEach items="${teamList }" var="team" varStatus="vs">
								<c:if test="${team.compId eq boardArticle.baCategory3 }"><b>${team.compNm }</b></c:if>
							</c:forEach>
						</c:if>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						${boardArticle.baCustomField1}
					</td>
					<th scope="row">${boardConfig.bcCustomField2 }</th>
					<td>
						${boardArticle.baCustomField2}
					</td>
				</tr>
				<tr>
					<th scope="row">${boardConfig.bcCustomField3 }</th>
					<td colspan="3">
						${boardArticle.baCustomField3}
					</td>
				</tr>
				<tr>
					<td colspan="4">
