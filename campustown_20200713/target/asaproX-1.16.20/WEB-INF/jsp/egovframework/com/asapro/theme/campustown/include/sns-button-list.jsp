	...
	jQuery(function(){
		 $('#facebookLink').click(function(e){
		  ...
		  $('#twLink').click(function(e){
			e.preventDefault();
			window.open('//twitter.com/home?status=<c:out value="${seo.title}" escapeXml="true" /> <%= AsaproUtils.getCurrentUrl(request, true, true) %>', 'share_popup', 'width=600,height=400');
			return false;
		 });
	});
	...
	function sendLink() {
	    Kakao.Link.sendDefault({
	         ...
	         content: {
	           ...
	           description: '<c:out value="${seo.description}" escapeXml="true" />',
