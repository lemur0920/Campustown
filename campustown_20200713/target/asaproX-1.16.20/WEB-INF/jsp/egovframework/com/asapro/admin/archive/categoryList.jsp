...
jQuery(function($){
    ...
    $('#deleteBtn').on('click', function () {
        ...
        swal({
        	...
        	$.ajax({
				url : '${ADMIN_PATH}/archive/category/delete.do'
				, type : 'post'
				, dataType: 'json'
				, data: {
					catIds : catIds
					...
					<div class="row">
					    <div class="col-sm-12">
					        <div class="card m-b-15">
					            <div class="card-body">
						            <form:form modelAttribute="archiveCategorySearch" cssClass="" action="${ADMIN_PATH}/archive/category/list.do" method="get">
										<div class="form-row mr-auto mt-2 form-inline">
											...
											<form:select path="sortOrder" cssClass="form-control mr-sm-2 mb-2" title="정렬기준">
												<form:option value="CAT_REGDATE" label="등록일" />
												<form:option value="CAT_NAME" label="신청자명" />
											</form:select>
										...
										<div class="form-row mt-2 form-inline" >
											...
											<form:select path="sc" cssClass="form-control mr-sm-2 mb-2" title="검색조건">
												...
												<form:option value="CAT_NAME" label="카테고리명" />
											</form:select>	
							...
							<div class="card m-b-30">
								<div class="card-body">
									...
									<div class="table-responsive">
										<table class="table table-striped table-hover ">
											<colgroup>
												...
												<col  /><!-- 아카이브타입 -->
												<col  /><!-- 카테고리명 -->
												<col style="width: 8%;" /><!-- 바로가기 -->
												<col  /><!-- 메타코드1 -->
												<col  /><!-- 메타코드2 -->
												<col  /><!-- 메타코드3 -->
												<col style="width: 8%;" /><!-- 자료수 -->
												<col style="width: 8%;" /><!-- 목록 -->
											...
											<tbody>
												...
												<c:forEach items="${paging.result}" var="item" varStatus="vs">
													<tr>
														...
														<td>
															<c:forEach items="${archiveItemList}" var="archiveItem">
																<c:if test="${archiveItem.customType == item.catCustomType}">${archiveItem.label}</c:if>
															</c:forEach>
														</td>
														<td><a href="${ADMIN_PATH}/archive/category/update.do?catId=${item.catId}" class=""><strong>${item.catName}</strong></a></td>
														<td>
															<c:if test="${not empty currentSite.seperatedSiteDomains}">
																<a href="//${currentSite.seperatedSiteDomains[0]}${APP_PATH}/archive/${item.catCustomType }/${item.catId}" class="btn btn-outline-secondary waves-effect btn-sm" target="_blank">바로가기</a>
															</c:if>
															<c:if test="${empty currentSite.seperatedSiteDomains}">
																<a href="${APP_PATH}/archive/${item.catCustomType }/${item.catId}" class="btn btn-outline-secondary waves-effect btn-sm" target="_blank">바로가기</a>
															</c:if>
														</td>
														<td>
															<c:forEach items="${codeCatList}" var="codeCat">
																<c:if test="${codeCat.catId == item.catMetaCode1}">${codeCat.catName}(${codeCat.catId})</c:if>
															</c:forEach>
														</td>
														<td>
															<c:forEach items="${codeCatList}" var="codeCat">
																<c:if test="${codeCat.catId == item.catMetaCode2}">${codeCat.catName}(${codeCat.catId})</c:if>
															</c:forEach>
														</td>
														<td>
															<c:forEach items="${codeCatList}" var="codeCat">
																<c:if test="${codeCat.catId == item.catMetaCode3}">${codeCat.catName}(${codeCat.catId})</c:if>
															</c:forEach>
														</td>
														<td>${item.catArchiveCount}</td>
														<td><a href="${ADMIN_PATH}/archive/${item.catCustomType }/list.do?arcCategory=${item.catId}" class="btn btn-outline-warning waves-effect waves-light btn-sm">목록</a></td>
														<td>
									...
									<div class="form-inline">
										<div class="mr-auto">
											<a href="${ADMIN_PATH}/archive/category/insert.do" class="btn btn-primary waves-effect waves-light">추가</a>
