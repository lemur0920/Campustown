...
jQuery(function($){
    ...
    $('#deleteBtn').on('click', function () {
    	var etIds = [];
		$('[name=etIds]:checked').each(function(idx, el){
			etIds.push(el.value);
		});
        ...
        swal({
        	...
        	$.ajax({
				url : '${ADMIN_PATH}/emailTarget/delete.do'
				, type : 'post'
				, dataType: 'json'
				, data: {
					etIds : etIds
					...
					<div class="row">
					    <div class="col-sm-12">
					        <div class="card m-b-15">
					            <div class="card-body">
						            <form:form modelAttribute="emailTargetInfoSearch" cssClass="form-inline" action="${ADMIN_PATH}/emailTarget/list.do" method="get">
										<div class="form-row mr-auto mt-2">
											...
											<form:select path="sortOrder" cssClass="form-control mr-sm-2 mb-2" title="정렬기준">
												<form:option value="ET_REGDATE" label="등록일시" />
												<form:option value="ET_TITLE" label="제목" />
											</form:select>
							...
							<div class="card m-b-30">
								<div class="card-body">
												...
												<tr>
													<th class="text-center">
														<div class="custom-control custom-checkbox">
															<input type="checkbox" class="custom-control-input" id="selectAll" onclick="jQuery('[name=etIds]').each(function(idx,el){this.checked=jQuery('#selectAll')[0].checked;});"/>
												...
												<c:forEach items="${paging.result}" var="item" varStatus="vs">
													<tr>
														<td class="text-center">
															<div class="custom-control custom-checkbox">
																<input type="checkbox" name="etIds" value="${item.etId}" class="custom-control-input" id="etIds${vs.count }" />
																<label class="custom-control-label" for="etIds${vs.count }"></label>
															</div>
														</td>
														<td>${asapro:codeName(item.etCate, etCateCodeList) }</td>
														<td><a href="${ADMIN_PATH}/emailTarget/update.do?etId=${item.etId}" class=""><strong>${item.etId}</strong></a></td>
														<td><a href="${ADMIN_PATH}/emailTarget/update.do?etId=${item.etId}" class="">${item.etTitle}</a></td>
														<td>
															<fmt:formatDate value="${item.etRegDate}" pattern="yyyy-MM-dd" />
														</td>
													</tr>
												</c:forEach>
											</tbody>
									...
									<div class="form-inline">
										<div class="mr-auto">
											<a href="${ADMIN_PATH}/emailTarget/insert.do" class="btn btn-primary waves-effect waves-light">메일대상추가</a>
