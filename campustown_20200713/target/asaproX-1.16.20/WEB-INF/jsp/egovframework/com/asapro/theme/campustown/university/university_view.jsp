...
jQuery(function($){
	<c:if test="${boardConfig.bcSupportComment}">
		...
		var cmtTitle = '${boardArticle.baTitle}';
	vi	var param = 'cmtModule=' + cmtModule + '&cmtModuleSub=' + cmtModuleSub +'&cmtModCategory=' + cmtModCategory + '&cmtModItemId=' + cmtModItemId + '&cmtTitle=' + cmtTitle;
...
<c:import url="${design.themeDir}/layout/sidebar3.jsp" charEncoding="UTF-8" />
	...
	<div class="view-table table-con">
		<table>
			...
			<tbody>
				...
				<tr>
					<th scope="row">제목</th>
					<td colspan="3">${boardArticle.baTitle}</td>
				</tr>
				<tr>
					<th scope="row">등록일</th>
					<td colspan="3"><fmt:formatDate value="${boardArticle.baRegDate}" pattern="yyyy-MM-dd" /></td>
				</tr>
				<tr>
					<th scope="row">조회수</th>
					...
					<%-- <th scope="row">좋아요</th>
					<td>${boardArticle.baRecommend}</td> --%>
				</tr>
				<tr>
					<td colspan="4">
	...
	<div class="btn-area txr clearfix">
		...
		<a class="btn01" href="${APP_PATH}/university/university_news?univId=${boardArticle.depId }">목록</a>
	</div>
