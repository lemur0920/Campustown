...
public static StringBuilder buildMenuMarkup(List<Menu> menuList, Menu menu, List<Menu> currentMenuList, Menu currentMenu, int snbDisplayDepth, StringBuilder sb){
	...
	for(Menu locationMenu : currentMenuList){
		if(locationMenu.getMenuDepth() == currentMenu.getMenuDepth() - 1){
			if(locationMenu.getMenuChildren() != null && !locationMenu.getMenuChildren().isEmpty() ){
				int i = 1;
				sb.append("<ul class=\"list_area_tit nsq clearfix\">");
...
//						}
					...
					//최초 메뉴의 순서가 유지된다 보고 i값을 같은 뎁스의 수만큼 증가시킨다.
					i += 1;
				}
				sb.append("</ul>");
...
int snbDisplayDepth = Integer.parseInt( StringUtils.defaultString("3", "3") );	//탭메뉴 3depth 임의 처리
...
for( Menu m1 : topMenuList ){
	if( topAncestorMenu != null ){
		if( currentMenu.getMenuDepth() > 2 && m1.getMenuId().equals(topAncestorMenu.getMenuId()) ){//해당 3뎁스메뉴의 부모 2뎁스 메뉴를 전달하여 자식메뉴 3뎁스만 출력
