...
jQuery(function($){
    ...
    $('#deleteBtn').on('click', function () {
    	var contentRoots = [];
		$('[name=contentRoots]:checked').each(function(idx, el){
			contentRoots.push(el.value);
		});
        	...
        	$.ajax({
				...
				if(result.success){
					swal({
		                type: 'success',
		                title: '삭제가 완료되었습니다.',
		                html: '삭제결과 : ' + result.successCnt + ' 레코드',
		                //showCancelButton: true,
	                    //confirmButtonClass: 'btn btn-success',
	                    //cancelButtonClass: 'btn btn-danger m-l-10'
		            }).then(function(){
					...
					<div class="row">
					    <div class="col-sm-12">
					        <div class="card m-b-15">
					            <div class="card-body">
						            <form:form modelAttribute="contentSearch" cssClass="form-inline" action="${ADMIN_PATH}/content/list.do" method="get">
							...
							<div class="card m-b-30">
								<div class="card-body">
												...
												<tr>
													<th class="text-center">
														<div class="custom-control custom-checkbox">
															<input type="checkbox" class="custom-control-input" id="selectAll" onclick="jQuery('[name=contentRoots]').each(function(idx,el){this.checked=jQuery('#selectAll')[0].checked;});"/>
												...
												<c:forEach items="${paging.result}" var="item" varStatus="vs">
													<tr>
														<td class="text-center">
															<div class="custom-control custom-checkbox">
																<input type="checkbox" name="contentRoots" value="${item.contentRoot}" class="custom-control-input" id="contentRoots${vs.count }" />
																<label class="custom-control-label" for="contentRoots${vs.count }"></label>
															</div>
														</td>
														<td>${paging.rowTop - vs.index}</td>
														<td><a href="${ADMIN_PATH}/content/update.do?contentId=${item.contentId}" class=""><strong>${item.contentTitle}</strong></a></td>
									...
									<div class="form-inline">
										<div class="mr-auto">
											<a href="${ADMIN_PATH}/content/insert.do" class="btn btn-primary waves-effect waves-light">추가</a>
