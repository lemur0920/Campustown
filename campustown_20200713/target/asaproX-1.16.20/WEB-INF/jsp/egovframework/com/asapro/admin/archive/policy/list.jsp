					...
					<div class="row">
					    <div class="col-sm-12">
					        <div class="card m-b-15">
					            <div class="card-body">
						            <form:form modelAttribute="policySearch" cssClass="" action="${ADMIN_PATH}/archive/policy/list.do" method="get">
										...
										<div class="form-row mr-auto mt-2 form-inline">
											...
											<form:select path="sortOrder" cssClass="form-control mr-sm-2 mb-2" title="정렬기준">
												...
												<form:option value="ARC_RECOMMEND" label="추천순" />
											</form:select>
										...
										<div class="form-row mt-2 form-inline" >
											...
											<%-- 
											<form:select path="resYear" cssClass="form-control mr-sm-2 mb-2" title="지원연도">
							...
							<div class="card m-b-30">
								<div class="card-body">
											...
											<tbody>
												...
												<c:forEach items="${paging.result}" var="item" varStatus="vs">
													<tr>
														...
														<c:if test="${not empty archiveCategory.catMetaCode1 }">
															<td>
																<c:forTokens items="${item.arcMetaCode1 }" delims="," var="mete1" varStatus="vs">
																	${asapro:codeName(mete1, arcMetaCode1List)}
														...
														<c:if test="${not empty archiveCategory.catMetaCode2 }">
															<td>
																<c:forTokens items="${item.arcMetaCode2 }" delims="," var="mete2" varStatus="vs">
																${asapro:codeName(mete2, arcMetaCode2List)}
																<c:if test="${!vs.last }">,</c:if>
																</c:forTokens>
															</td>
														</c:if>
														<c:if test="${not empty archiveCategory.catMetaCode3 }">
															<td>
																<c:forTokens items="${item.arcMetaCode3 }" delims="," var="mete3" varStatus="vs">
																${asapro:codeName(mete3, arcMetaCode3List)}
																<c:if test="${!vs.last }">,</c:if>
																</c:forTokens>
															</td>
														</c:if>
														...
														<td>
															<a href="${ADMIN_PATH}/archive/policy/update.do?arcId=${item.arcId}" class=""><strong>${item.arcTitle}</strong></a>
															<c:if test="${not empty currentSite.seperatedSiteDomains}">
																<a href="//${currentSite.seperatedSiteDomains[0]}${item.permLink}?metaCode1=${item.arcMetaCode1}&metaCode2=${item.arcMetaCode2}&metaCode3=${item.arcMetaCode3}" class="badge badge-success" target="_blank" title="보기"> 바로가기</a>
															</c:if>
															<c:if test="${empty currentSite.seperatedSiteDomains}">
																<a href="${item.permLink}?metaCode1=${item.arcMetaCode1}&metaCode2=${item.arcMetaCode2}&metaCode3=${item.arcMetaCode3}" class="badge badge-success" target="_blank" title="보기"> 바로가기</a>
															</c:if>
														</td>
														<td>
														...
														<td>
															<c:if test="${item.arcUse}"><span class="badge badge-success">게시</span></c:if>
									...
									<div class="form-inline">
										<div class="mr-auto">
											<a href="${ADMIN_PATH}/archive/policy/insert.do?arcCategory=${archiveCategory.catId}" class="btn btn-primary waves-effect waves-light">추가</a>
